...
| Pieza | Estado |
|---|---|
| `frontend/` — `ciudadano-web` | **El andamiaje (issue 1), el tema (issue 2) y los datos de demostración (issue 3), sin pantallas.** Vite 7 + React 19.3 + TypeScript 5.9 con yarn classic. `yarn verificar` encadena lint, tipos, i18n y pruebas: **20 archivos, 192 pruebas**, 0 fallos. `yarn build` construye el bundle en `dist/`. `yarn dev` sirve `http://localhost:5174/portal/` con un marcador —la barra del artboard: escudo, «Pago de tributos en línea» y «Municipalidad Distrital de Catacaos» por `t()`, en blanco sobre `bg-azul`, sobre `bg-fondo`— |
...
| Pantallas | **No existen todavía.** Estado del recorrido: issue 4. Arnés de Playwright: issue 11 |
