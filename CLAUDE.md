...
| Pieza | Estado |
|---|---|
| `frontend/` — `ciudadano-web` | **El andamiaje (issue 1), el tema (issue 2), los datos de demostración (issue 3) y el recorrido con su marco (issue 4); los pasos son marcadores.** Vite 7 + React 19.3 + TypeScript 5.9 con yarn classic. `yarn verificar` encadena lint, tipos, i18n y pruebas: **24 archivos, 234 pruebas**, 0 fallos. `yarn build` construye el bundle en `dist/`. `yarn dev` sirve `http://localhost:5174/portal/#/buscar`: la barra, la franja de pasos y el pie del artboard, y en cada paso un marcador con su título |
...
| Pantallas | **Todavía marcadores.** Cada paso, su título: issues 5-10. Arnés de Playwright: issue 11 |
