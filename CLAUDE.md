...
| Pieza | Estado |
|---|---|
| `frontend/` — `ciudadano-web` | **El andamiaje (issue 1), el tema (issue 2), los datos de demostración (issue 3), el recorrido con su marco (issue 4) y las pantallas de buscar (issue 5), de elegir qué pago (issue 6), de «Mis datos» (issue 7) y de pagar (issue 8); los demás pasos son marcadores.** Vite 7 + React 19.3 + TypeScript 5.9 con yarn classic. `yarn verificar` encadena lint, tipos, i18n y pruebas: **33 archivos, 307 pruebas**, 0 fallos. `yarn build` construye el bundle en `dist/`. `yarn dev` sirve `http://localhost:5174/portal/#/buscar`: la barra, la franja de pasos y el pie del artboard, la pantalla de buscar en `#/buscar`, la de elegir qué pago en `#/deudas`, la de «Mis datos» en `#/identificar`, la de pagar en `#/pagar`, y en los demás pasos un marcador con su título |
...
frontend/                 Vite 7, React 19, TypeScript 5.9, con yarn. `ciudadano-web`
  ...
  src/pasos/              una carpeta por pantalla del recorrido (`buscar/`, issue 5; `deudas/`, issue 6; `identificar/`, issue 7; `pagar/`, issue 8)
