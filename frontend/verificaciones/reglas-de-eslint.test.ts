 ...
 *
 * Es el equivalente frontend de `ReglasDeArquitecturaMuerdenTest` del backend de `rentas`: cada prohibicion tiene
 ...
 *
 *   1. La lista de prohibiciones **se importa del propio config**. No hay copia — y desde rentas#137
 ...
 *
 * **Medido en `rentas` (rentas#36): el arranque en frio costaba 3,64 s y sus diecinueve hermanas, de 0,02 a
...
describe('las reglas no senalan codigo correcto', () => {
  ...
  it('el codigo que las respeta pasa limpio', async () => {
    // Adaptado a este portal: sin cliente HTTP, los importes llegan de los datos de demostracion
    // como texto decimal y se suman con `sumarImportes`, que es lo que las reglas piden.
    const correcto = `
      ...
      export function totalDe(recibos: readonly Recibo[]) {
        return sumarImportes(recibos.map((recibo) => recibo.total));
      }
    `;
