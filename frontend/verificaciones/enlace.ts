 ...
 *
 * Portada de `rentas/frontend/verificaciones/enlace.ts` sin cambiar la logica. Las mediciones que
 * citan los comentarios se hicieron alli, y se dejan con su procedencia porque son el motivo de
 * que el codigo sea como es.
 *
 ...
 * salir** en `rentas`. Con el clon hermano ausente, `require.resolve('@kamayuk/api')` revienta
 * doce lineas antes, en la primera vuelta del bucle, y lo que se lee es:
 *
 ...
 *
 * Aqui son tres enlaces (seis en `rentas`). Un rojo que dijera «algun `@kamayuk/*` no resolvio» obliga a
 ...
 *
 * Es exactamente de lo que el backend de `rentas` se defiende desde `settings.gradle.kts`, que se
 * para ANTES y dice el `git clone` que falta.
 */
