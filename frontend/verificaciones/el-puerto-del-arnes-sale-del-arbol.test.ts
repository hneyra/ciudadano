...
/**
 * **El puerto del arnes sale del arbol de trabajo, y si esta ocupado se dice.** Portada de
 * `rentas/frontend/verificaciones/el-puerto-del-arnes-sale-del-arbol.test.ts` (rentas#148) en el
 * issue 11, con la base `/portal/`.
 *
 * <h2>Que defecto vigila</h2>
 *
 * El de `rentas`: `playwright.config.ts` escribia el 4173 en tres sitios, y con varios worktrees a la
 * vez `yarn e2e` de una rama midio el bundle de otra —rentas#140, `Received: 0`—. Las pruebas de aqui
 * se ponen rojas si el puerto vuelve a ser una constante, si deja de depender del arbol, o si la
 * comprobacion de ocupacion mira una sola cara de `localhost`.
 *
 * <h2>Por que hay una prueba que abre sockets</h2>
 *
 * Porque `vite preview` escucha en `[::1]` y NO en `127.0.0.1` (medido en rentas#148), asi que una
 * comprobacion que solo mirase la cara IPv4 diria «libre» de un puerto que no lo esta. Eso no lo
 ...
 * defecto de rentas#140 y por que el puerto dejo de ser una constante. Borrar esa historia para que
...
const ARBOLES = [
  '/home/jorge/ws/ciudadano/frontend',
  '/home/jorge/ws/ciudadano-10/frontend',
  '/home/jorge/ws/ciudadano-11/frontend',
  '/home/jorge/ws/rentas/frontend',
  '/home/jorge/ws/rentas-148/frontend',
  '/tmp/obra/ciudadano/frontend',
];
...
describe('el puerto del arnes', () => {
  it('EL CENTINELA: `playwright.config.ts` no escribe NINGUN puerto a mano', () => {
    // Esta es la prueba que se pone roja el dia que alguien fije el 4173 —o cualquier otro— en el
    // config. No comprueba un numero: comprueba que no haya numero.
  ...
  it('y `yarn e2e` lo comprueba ANTES de arrancar Playwright', () => {
    // El orden es la mitad de la guarda. Medido en rentas#148 con un intruso en el puerto derivado:
    // puesta dentro de `webServer.command`, la comprobacion llega tarde —Playwright ya ha dicho lo
  ...
  it('y el del mismo arbol es el mismo siempre: es lo que se puede nombrar al depurar', () => {
    // Un puerto libre de verdad daria otro en cada corrida —medido en rentas#148: diez sondeos,
    // diez puertos—, y entonces un `vite preview` colgado de ayer no estorbaria a nadie nunca,
    // que es como se acumulan en la misma maquina sin que nadie se entere.
  ...
  it('cae entre los dos puertos de Vite, sin pisar ninguno de los dos', () => {
    for (const arbol of [...ARBOLES, AQUI]) {
      ...
      expect(puerto).toBeLessThan(5173); // `yarn dev` de `rentas`; el de este portal es el 5174
    }
  });

  it('en CI es el de siempre, porque alli solo hay un arbol', () => {
    expect(elegirElPuerto({ raiz: '/home/runner/work/ciudadano/ciudadano/frontend', ci: true })).toEqual({
      puerto: 4173,
  ...
  it('fuera de CI NUNCA es el de CI, aunque el arbol este en la misma ruta', () => {
    expect(
      elegirElPuerto({ raiz: '/home/runner/work/ciudadano/ciudadano/frontend', ci: false }).puerto,
    ).not.toBe(4173);
  });
  ...
  it('el de ESTE arbol es el que lleva la URL con la que se mide', () => {
    expect(URL_DEL_ARNES).toBe(`http://localhost:${PUERTO}/portal/`);
    // `/portal/` es la `base` de `vite.config.ts`: con otra, `preview` contesta 404 en la raiz.
...
describe('la comprobacion de que el puerto esta libre', () => {
  it('ve un servidor que solo escucha en `[::1]`, que es como escucha `vite preview`', async () => {
    ...
    // un puerto en el que `vite preview` no va a poder abrir (rentas#148, medido con `ss -ltnp`).
