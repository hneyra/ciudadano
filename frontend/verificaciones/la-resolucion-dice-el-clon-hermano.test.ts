...
/**
 * **El mensaje que nombra el `git clone` llega a salir** (rentas#113). Portada de `rentas`.
 *
 * <h2>Que se habia roto, y por que nadie lo vio</h2>
 *
 * En `rentas`, `resolucion.ts` terminaba con un `throw` escrito para el clon hermano ausente, con su
 * `git clone` dentro. Y era **inalcanzable justo en ese caso**: `require.resolve('@kamayuk/api')`
 * reventaba doce lineas antes, en la primera vuelta del bucle. Lo medido, con `node_modules/@kamayuk/`
...
/**
 * Una raiz de frontend que no existe, con la disposicion de la de verdad: `…/ciudadano/frontend`,
 * para que `../../kamayuk-lib` caiga al lado de «ciudadano» y no en cualquier sitio.
 */
const RAIZ_SIN_HERMANO = join(tmpdir(), 'kamayuk-ws-de-mentira', 'ciudadano', 'frontend');
...
describe('LA MUESTRA: sin el clon hermano, el rojo dice donde mirar', () => {
  const rojo = rojoDe(RAIZ_SIN_HERMANO, '@kamayuk/ui', '../../kamayuk-lib/paquetes/ui');

  it('nombra el paquete que no resolvio', () => {
    // Son TRES enlaces: «alguno de `@kamayuk/*`» obliga a probarlos a mano.
    expect(rojo).toContain('@kamayuk/ui');
  });

  it('nombra la ruta exacta donde se espera el clon', () => {
    expect(rojo).toContain(CLON_ESPERADO);
    expect(rojo).toContain(join(CLON_ESPERADO, 'paquetes', 'ui'));
  });

  it('y nombra el `git clone` que lo pone ahi', () => {
    // Lo unico que distingue este rojo del que salia antes —«Cannot find module '@kamayuk/ui'»,
  ...
  it('cada uno de los tres enlaces declarados se nombra a si mismo', () => {
...
describe('y si el clon ESTA, el remedio es otro y no miente', () => {
  it('manda a `yarn install`, no a clonar lo que ya esta clonado', () => {
    ...
    const rojo = rojoDe(FRONTEND, '@kamayuk/ui', '../../kamayuk-lib/paquetes/ui');
...
describe('con el hermano en su sitio, resuelve', () => {
  it('los tres enlaces se resuelven, y los que piden algo lo piden de verdad', () => {
