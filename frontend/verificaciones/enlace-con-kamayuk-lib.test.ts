...
/**
 * **El clon hermano, bien puesto — y si no lo esta, que se diga nombrando el `git clone`.**
 *
 * Portada de `rentas/frontend/verificaciones/enlace-con-kamayuk-lib.test.ts` (rentas#74).
 *
 ...
 * 2. **Que resuelva de verdad**, importando de los paquetes y ejecutandolos. Eso vive aparte, en
 *    `resuelve-el-clon-hermano.test.ts`, por lo que cuenta la ultima prueba de este archivo.
 */
...
describe('LA MUESTRA: la guarda muerde, y dice el `git clone`', () => {
  ...
  it('y si apunta al paquete EQUIVOCADO, que es lo que nadie mira', () => {
    ...
    writeFileSync(join(raiz, 'otro/package.json'), JSON.stringify({ name: '@kamayuk/ui' }));

    const problemas = problemasDelEnlace(raiz);

    expect(problemas[0]?.que).toBe('«./otro» es «@kamayuk/ui», no «@kamayuk/formato».');
  });
...
describe('LA GUARDA DE LA GUARDA: este archivo no importa de `@kamayuk/*`, y no puede', () => {
  it('cero imports del hermano en el archivo que avisa de que el hermano falta', () => {
    ...
    //
    // en la RECOLECCION — y con el, las pruebas de este archivo **desaparecieron**, la
