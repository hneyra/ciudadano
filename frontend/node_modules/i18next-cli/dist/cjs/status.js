...
async function generateStatusReport(config) {
    ...
    const fallbackNamespaces = !fallbackNS
        ? []
        : (Array.isArray(fallbackNS) ? fallbackNS : [fallbackNS]).filter((ns) => typeof ns === 'string' && ns.length > 0);
    ...
    const nsSep = config.extract.nsSeparator ?? ':';
    const ignoreKeyMatchers = (config.status?.ignoreKeys ?? []).map((pattern) => {
        ...
        return (k, keyNs) => (ns === undefined || ns === keyNs) && re.test(k.key);
    });
            ...
            const resolveAndClassify = (key) => {
                ...
                if (primaryState === 'absent') {
                    for (const fallbackTranslations of fallbackTranslationsList) {
                        const fallbackState = classifyValue(nestedObject.getNestedValue(fallbackTranslations, key, sep));
