        ...
        for (const file of sourceFiles) {
            try {
                ...
                if (scanResult.moduleScopeSkipped > 0) {
                    ...
                    logger$1.warn(`${file}: skipped ${scanResult.moduleScopeSkipped} module-scope string(s). ` +
...
async function scanFileForCandidates(content, file, config) {
    ...
    try {
        ...
        const moduleScoped = candidates.filter(c => !functionSpans.some(span => c.offset >= span.start && c.endOffset <= span.end));
 ...
 */
function collectFunctionSpans(node, spans) {
