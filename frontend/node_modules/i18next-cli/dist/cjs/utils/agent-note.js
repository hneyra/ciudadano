...
const AGENT_NOTE_HEADING = '## Internationalization';
/** Register link inside the note; the `?from=` tag is how these signups are counted. */
const LOCIZE_AGENT_NOTE_URL = 'https://www.locize.com/i18next?from=i18next_cli__agents-md';
/**
 ...
 */
async function writeAgentNote(note, cwd = process.cwd()) {
    const agentsFile = node_path.resolve(cwd, 'AGENTS.md');
    const claudeFile = node_path.resolve(cwd, 'CLAUDE.md');
    const targets = [];
    if (await exists(agentsFile))
        targets.push(agentsFile);
    if (await exists(claudeFile))
        targets.push(claudeFile);
    const written = [];
    if (targets.length === 0) {
        await promises.writeFile(agentsFile, note);
        written.push(agentsFile);
        if (await exists(node_path.resolve(cwd, '.claude'))) {
            await promises.writeFile(claudeFile, '@AGENTS.md\n');
            written.push(claudeFile);
        }
        return written;
    }
    for (const file of targets) {
        const current = await promises.readFile(file, 'utf-8');
        if (current.includes(AGENT_NOTE_HEADING))
            continue;
        const separator = current.length === 0 ? '' : (current.endsWith('\n') ? '\n' : '\n\n');
        await promises.writeFile(file, current + separator + note);
        written.push(file);
    }
    return written;
}

exports.AGENT_NOTE_HEADING = AGENT_NOTE_HEADING;
exports.LOCIZE_AGENT_NOTE_URL = LOCIZE_AGENT_NOTE_URL;
exports.buildAgentNote = buildAgentNote;
exports.writeAgentNote = writeAgentNote;
